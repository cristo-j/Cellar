# Cellar
Aplicativo para controle de adega pessoal

# Cellar-API - Tarefa para Aprendizado

API desenvolvida como projeto para a disciplinas de desenvolvimento back-end, utilizando Javascript com Nodejs + [Express](https://expressjs.com/). O projeto consistia na elaboração de uma API para comunicar dados em JSON a respeito de uma modelagem. É uma API RESTful com validações de schemas JSON ([Ajv](https://ajv.js.org/)) e para autenticação utiliza JWT Token ([jsownwebtoken](https://github.com/auth0/node-jsonwebtoken)). Na persistência de dados, se utiliza o ORM [Sequelize](https://sequelize.org/) com banco de dados em MySQL (podendo ser alterado).
...
Este projeto faz referência a uma modelagem de uma de adega pessoal, da relação entre vinhos e suas garrafas (existentes na adega do usuário).

![Imagem da Modelagem inicial do sistema](/modelagem/Diagrama%20BD%20adega.pdf)
...
2. Para acessar o banco de dados, caso não queira manipular o BD via linha de comando, pode utilizar um ou mais softwares usuários, como [MySQL Workbench](https://www.mysql.com/products/workbench/) ou [DBeaver](https://dbeaver.io/). Deverá ser previamente criado um schema com o nome "adega" (a criação de tabelas e demais configurações serão feitas pela API).
...
Cada model do projeto (app/models) é uma classe com dados privados (comum em JS). Entretanto, na mesma classe, a Model do Sequelize para cada entidade é inicializada. A ideia é que métodos comuns do ORM como create, update, dentre outros, sejam acopladas a classe comum da model. Isto facilita caso futuramente seja necessário trocar o Sequelize por outro framework. Cada model tecnicamente exporta a sua classe e a model sequelize conectada, respectivamente.
...
Para a utilização da API é necessário ter um token de acesso que deverá ser enviado pelo lado do usuário, via cabeçalho da requisição, para esta API autorizar o consumo de dados. No cabeçalho deve ser especificado Bearer token, onde token é gerado para um usuário conhecido. Um middleware em app/middlewares tem por responsabilidade ser o intermediário, avaliando se existe o token na requisição e se o mesmo é válido.

O projeto segue o padrão de implementação de tokens via JWT Token. É necessário que um usuário que deseja utilizar a API faça o cadastro para se tornar um usuário conhecido e autorizado. Ao fazer o cadastro o token é gerado e retornado ao usuário. Por padrão cada token tem um tempo de validade definido em config.js (jwt.expiration).

Também existe a implementação de login para os usuários, ao enviar e-mail e senha corretos para esta API, a mesma verificará as credenciais e irá gerar um token. No final desta mesma requisição o usuário deverá receber um token de acesso para utilizar normalmente a API.
...
Serão aceitas atualizações de pacotes, correções de bugs e melhorias. Novas funcionalidades podem serem debatidas e adicionadas ao projeto.
